...
// ─── INPUT SANİTİZASYON ─────────────────────────────────────────────────────
/**
 * Sanitize single message against XSS, injection attacks
 * Uses professional XSS library + control character removal
 */
export function sanitizeMessage(content) {
    if (typeof content !== 'string') return '';
    if (content.length > 4000) content = content.substring(0, 4000);

    // Use professional sanitizer (replaces old regex approach)
    return sanitizeString(content);
}

/**
 * Sanitize message array (prevents array injection, validates roles)
 */
export function sanitizeMessages(messages) {
    if (!Array.isArray(messages)) return [];
    if (messages.length > 100) messages = messages.slice(-100); // Prevent memory exhaustion
    ...
    return messages.map(m => {
        ...
        // Sanitize content with XSS protection
