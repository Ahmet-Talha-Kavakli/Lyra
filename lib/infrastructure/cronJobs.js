...
import { sanitizeString, isValidUrl } from '../shared/validators.js';
        ...
        for (const category of categories.slice(0, 3)) {
            try {
                ...
                for (const src of newSources.slice(0, 2)) {
                    ...
                    // Validate URL (prevent SSRF attacks on internal IPs)
                    ...
                    try {
                        ...
                        logger.error('[CronJob] Database insert error', { error: dbErr.message, url: src.url });
                    }
                }
...
export async function assessKnowledgeQuality() {
    try {
        ...
        const { data: sources } = await supabase
            .from('knowledge_sources')
            .select('id, title, usage_count, helpful_feedback_count, credibility_score')
            .eq('is_active', true)
